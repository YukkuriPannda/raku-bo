...
**`.Codex/FILEMAP.md`** を参照すること。ディレクトリごとの役割に加えて、
...
**Codexが勝手にマージしないこと。** ブランチを `main` へマージする前に、
必ず本人（開発者）が実機で動作を確認する。Codexは実装・検証・コミット・
...
> 補足（Codex向け）: このプロジェクトは `react-native-android-widget` /
...
- コードを修正した後は必ず再起動をCodexが実行してください。
